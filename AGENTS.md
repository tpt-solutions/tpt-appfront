...
appfront-cli    — stub; future `appfront` CLI
```
...
- **examples/** is excluded from the workspace and gitignored — each has its own `Cargo.toml`/dependency resolution. CI's examples job uses `trunk build` on every `examples/*/index.html`.
