...
//! [`appfront_core::query_state`]/[`appfront_core::navigate_to`] plus an
//! app-supplied command handler (mirroring `appfront-server`'s
...
use appfront_core::{query_state, UITree};
...
/// immediate-mode pattern `appfront-canvas` uses.
...
impl<Msg> McpServer<Msg> {
    ...
    /// [`appfront_core::trigger_event`] or updating a `Signal` directly.
    pub fn new(
    ...
    fn call_tool(&self, name: &str, arguments: HashMap<String, Value>, id: Option<Value>) -> Value {
        let (is_error, text) = match name {
            ...
            "navigate" => match arguments.get("route").and_then(Value::as_str) {
                Some(route) => {
                    appfront_core::navigate_to(route);
