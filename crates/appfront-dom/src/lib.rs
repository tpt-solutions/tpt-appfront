...
use appfront_core::{reconcile_keys, create_effect, HydrationPayload, KeyedDiff, NodeKind, Router, UITree};
...
/// Drops every tracked closure for `node`, releasing the `Closure` once the
/// browser no longer holds the handler. Called from `unmount`.
fn drop_closures_for(node: &Node) {
    let id = node_id(node);
    LIVE_CLOSURES.with(|m| m.borrow_mut().remove(&id));
}
